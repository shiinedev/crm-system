...
import { openai, MODEL, MODEL_MINI } from "@/server/ai/client"
  ...
  generateEmail: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { text } = await generateText({
        model: openai(MODEL),
        prompt: `Write a professional, concise sales email to ${contact.firstName} ${contact.lastName}${contact.title ? `, ${contact.title}` : ""}.
  ...
  analyzeDeal: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { text } = await generateText({
        ...
        prompt: `Analyze this deal and give a brief risk assessment and next recommended action.
...
Reply with two short paragraphs: (1) Risk analysis, (2) Recommended next action. Max 80 words total.`,
  ...
  summarizeCompany: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { text } = await generateText({
        model: openai(MODEL_MINI),
        prompt: `Write a brief CRM summary for this company.
...
2-3 sentences max. Focus on sales relevance.`,
  ...
  scoreLead: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { text } = await generateText({
        model: openai(MODEL_MINI),
        prompt: `Score this lead from 0-100 based on available data. Reply with ONLY a number, nothing else.
